...
class app
{
	...
	public function questions()
	{
		...
		switch($subaction)
		{
			...
			if($search['knowsids'])
			{
				$args[] = array('AND',"questions.questionknowsid IN :questionknowsid",'questionknowsid',$search['knowsids']);
			}
			if($search['stime'])
			{
	...
	public function users()
	{
		...
		switch($subaction)
		{
			...
			$page = $page > 0?$page:1;
			$exams = $this->favor->getExamHistoryListByArgs($page,30,array(array('AND',"ehstatus = '1'"),array('AND',"ehbasicid = :ehbasicid",'ehbasicid',$basicid)));
