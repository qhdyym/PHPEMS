	...
	public function lesson()
	{
		...
		switch($action)
		{
			...
			switch($this->ev->url(4))
			{
				...
				if($questype && $knowsid)
				{
					$tquestid = $this->ev->getCookie('questype');
					$tknowsid = $this->ev->getCookie('knowsid');
					if(($tquestid != $questype) || ($tknowsid != $knowsid))
					{
						$this->ev->setCookie('questype',$questype,3600*24);
						$this->ev->setCookie('knowsid',$knowsid,3600*24);
						$this->ev->setCookie('number',0);
					}
					$message = array(
						'statusCode' => 200,
					    "callbackType" => 'forward',
	...
	public function exampaper()
	{
		...
		switch($action)
		{
			...
			else
			{
				...
				{
					$sessionvars = $this->exam->getExamSessionBySessionid();
					$args['examsessionquestion'] = array('questions'=>$r['examquestions'],'questionrows'=>array());
					$args['examsessionsetting'] = $r;
					$args['examsessionstarttime'] = TIME;
					$args['examsession'] = $r['exam'];
					$args['examsessionscore'] = 0;
